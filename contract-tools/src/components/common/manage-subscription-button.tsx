...
export function ManageSubscriptionButton({ className }: { className?: string }) {
  ...
  const handleClick = async () => {
    const customerId = typeof window !== 'undefined' ? localStorage.getItem('contract_customer_id') : null;
    if (!customerId) {
  ...
  return (
    <Button onClick={handleClick} disabled={loading} variant="outline" className={className}>
      {loading ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" />処理中...</> : 'サブスクリプション管理'}
    </Button>
  );
}
