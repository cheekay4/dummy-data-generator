// Gmail用コンテントスクリプト（Shadow DOM インラインUI方式）
...
const INLINE_PANEL_ID = 'keigo-inline-panel';

// ---- Shadow DOM インラインUI ----

function buildInlinePanel(hostEl: HTMLElement): ShadowRoot {
  hostEl.id = INLINE_PANEL_ID;
  hostEl.style.cssText = 'display:block; margin-top:8px;';
...
function injectButton(composeDialog: Element): void {
  const toolbar = composeDialog.querySelector(GMAIL_SELECTORS.toolbar);
  if (!toolbar) return;
  if (toolbar.querySelector(`[${GMAIL_SELECTORS.injectedAttr}]`)) return;

  const btnCell = document.createElement('td');
  btnCell.setAttribute(GMAIL_SELECTORS.injectedAttr, 'true');
  ...
  btn.addEventListener('click', (e) => {
    e.stopPropagation();
    toggleInlinePanel(composeDialog, btn);
  });
...
function toggleInlinePanel(composeDialog: Element, triggerBtn: HTMLElement): void {
  // 拡張機能コンテキストが無効（拡張機能リロード後にページ未リフレッシュ）の場合
  if (!isContextValid()) {
  ...
  // メール本文テキストを取得
  const bodyEl = composeDialog.querySelector<HTMLElement>(GMAIL_SELECTORS.bodyEditable);
  const mailText = bodyEl?.innerText?.trim() ?? '';

  // フローティングパネルをdocument.bodyに挿入（作成エリアのDOM構造に影響しない）
  const host = document.createElement('div');
  const shadow = buildInlinePanel(host);

  // ボタン位置を基準にパネルを配置（ボタンの上に表示）
  ...
  btnPaste.addEventListener('click', () => {
    if (!bodyEl) return;
    const converted = resultArea.textContent ?? '';
    // 既存テキストを変換結果で置き換え
    bodyEl.innerText = converted;
...
function scanAndInject(): void {
  const dialogs = document.querySelectorAll(GMAIL_SELECTORS.composeDialog);
  dialogs.forEach((dialog) => {
    if (dialog.querySelector(GMAIL_SELECTORS.bodyEditable)) {
      injectButton(dialog);
    }
  });
}
