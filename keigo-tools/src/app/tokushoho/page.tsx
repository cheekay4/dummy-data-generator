...
const ITEMS = [
  ...
  { label: '販売価格', value: 'Proプラン 月額290円（税込）' },
  ...
  { label: '解約方法', value: 'マイページ（料金ページ）からいつでも解約可能' },
  {
    label: '返金ポリシー',
    value:
      'デジタルサービスの性質上、原則として返金はお受けしておりません。ご不明な点はお問い合わせください。',
  },
          ...
          <tbody>
            {ITEMS.map((item, i) => (
              <tr key={item.label} className={i > 0 ? 'border-t' : ''}>
                <th className="px-4 py-3 text-left font-medium bg-muted/40 align-top whitespace-nowrap w-40">
                  {item.label}
                </th>
