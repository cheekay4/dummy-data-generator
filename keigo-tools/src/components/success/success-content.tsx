  ...
  useEffect(() => {
    ...
    fetch(`/api/session?session_id=${encodeURIComponent(sessionId)}`)
      ...
      .then(({ customerId, subscriptionId, error }) => {
        ...
        if (subscriptionId) {
          localStorage.setItem('keigo_subscription_id', String(subscriptionId));
        }
        if (customerId) {
          localStorage.setItem('keigo_customer_id', String(customerId));
        }
        setStatus('success');
      })
  ...
  return (
    <div className="max-w-md mx-auto py-24 text-center space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-2">敬語メールライターが無制限でご利用いただけます。</p>
      </div>
