...
export const metadata: Metadata = {
  ...
  openGraph: {
    ...
    url: "https://tools24.jp/dev/cron-expression-builder",
    type: "website",
  },
  alternates: {
    canonical: "https://tools24.jp/dev/cron-expression-builder",
  },
  other: {
...
const faqs = [
  {
    ...
    answer:
      "はい、完全無料です。会員登録も不要で、全ての機能を無制限でご利用いただけます。",
  },
  {
    question: "データはサーバーに送信されますか？",
    answer:
      "いいえ、全てブラウザ内で処理されます。入力したデータは外部に一切送信されません。",
  },
  {
    question: "JSTとUTCの違いは何ですか？",
    answer:
      "JST（日本標準時）はUTCより9時間進んでいます。多くのクラウドサーバー（AWS、GCPなど）はUTCで動作するため、「JSTの午前9時に実行」したい場合はcron式の時フィールドに「0」（UTC 0時 = JST 9時）を設定します。",
  },
  {
    question: "月の最終日に実行するにはどうすればいいですか？",
    answer:
      "標準的なcron式には「最終日」を表す直接的な記法がありません。28日（全月に存在）を使うか、cronの拡張記法をサポートするシステム（AWSのEventBridgeなど）では「L」記法が使えます。",
  },
  {
    question: "GitHub ActionsのCronはどのタイムゾーンですか？",
    answer:
      "GitHub ActionsのCronはUTC基準です。JSTの午前9時（9:00 JST）に実行したい場合は「0 0 * * *」（UTC 0:00）と設定します。",
  },
];
...
export default function CronExpressionBuilderPage() {
  return (
    <div className="container mx-auto px-4 py-6">
      ...
      {/* FAQ（FAQSection共通コンポーネント使用） */}
      <FAQSection faqs={faqs} />

      {/* 広告: フッター上 */}
      <AdPlaceholder position="content" />
      <AffiliateTextLink {...affiliateDevTools['/dev/cron-expression-builder']} />
    </div>
  );
}
