...
| **メッセージ整合性** | **LP・チュートリアル・ヘルプテキスト・ボタンラベル等の文言が、プロダクトの実際の挙動・数値・コンセプトと一致しているか（例: 「3回無料」と書いてあるのに実際は1回、推し機能が説明文ではおまけ扱いになっている、等）** |
| **クリッカブル錯覚** | **リンクでないテキストが、矢印（→）・下線・色変化などでクリック可能に見えないか。逆に、リンクなのにリンクに見えないテキストがないか** |
